pipeline {
    ...
    stages {
		...
		sh "docker run --name zapcontainer -u root -v $PWD:/zap/wrk -t owasp/zap2docker-weekly zap-baseline.py -t http://192.168.1.170:8080 -g gen.conf -r owaspzap.html || true"
