        ...
        stage('Build') { 
            steps {
                ...
                sh "echo JOB_URL  $JOB_URL"
              	sh "docker run -u root -v $JENKINS_HOME/jobs/$JOB_NAME/builds/$BUILD_NUMBER:/zap/wrk:rw -t owasp/zap2docker-weekly zap-baseline.py -t https://www.example.com -g gen.conf -r jj3.html || true"
  	    }
        }
    }
